...
def fit_psychfunc(df):
    choicedat = df.groupby('signed_contrast').agg({'choice':'count', 'choice2':'mean'}).reset_index()
...
def plot_psychometric(x, y, subj, **kwargs):
	...
	df = pd.DataFrame({'signed_contrast':x, 'choice':y, 'choice2':y, 'subject_nickname':subj})
	df2 = df.groupby(['signed_contrast', 'subject_nickname']).agg({'choice2':'count', 'choice':'mean'}).reset_index()
	...
	# plot psychfunc
	sns.lineplot(np.arange(-29, 29), psy.erf_psycho_2gammas(pars, np.arange(-29, 29)), **kwargs)

	# plot psychfunc: -100
	sns.lineplot(np.arange(-37, -32), psy.erf_psycho_2gammas(pars, np.arange(-103, -98)), **kwargs)
	sns.lineplot(np.arange(32, 37), psy.erf_psycho_2gammas(pars, np.arange(98, 103)), **kwargs)
	...
	df3 = df.groupby(['signed_contrast', 'subject_nickname']).agg({'choice2':'count', 'choice':'mean'}).reset_index()
...
def plot_chronometric(x, y, subj, **kwargs):
	...
	sns.lineplot(x='signed_contrast', y='rt', err_style="bars", mew=0.5, 
...
def add_n(x,y,sj,**kwargs):
	...
	df = pd.DataFrame({'signed_contrast':x, 'choice':y, 'choice2':y,'subject_nickname':sj})
...
def dj2pandas(behav):
	...
	behav['signed_contrast'] = (behav['trial_stim_contrast_right'] - behav['trial_stim_contrast_left']) * 100
	...
	behav['correct'] = np.where(np.sign(behav['signed_contrast']) == behav['choice'], 1, 0)
	behav.loc[behav['signed_contrast'] == 0, 'correct'] = np.NaN

	behav['choice_right'] = behav.choice.replace([-1, 0, 1], [0, np.nan, 1])  # code as 0, 100 for percentages
