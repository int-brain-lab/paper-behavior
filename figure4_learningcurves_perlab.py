...
behav['session_day'] = behav.groupby(['lab_name', 'subject_nickname'])['session_date'].cumcount()
...
fig.map(plot_learningcurve, "session_day", "performance_easy", "subject_nickname")
...
fig.savefig(os.path.join(figpath, "figure4e_learningcurves_perlab.png"), dpi=600)
plt.close('all')

fig = sns.FacetGrid(behav,
	col="subject_nickname", col_wrap=8, hue="lab_name", palette="colorblind",
					sharex=True, sharey=True, aspect=1)
fig.map(plot_learningcurve, "session_day", "performance_easy", "subject_nickname").add_legend()
fig.set_axis_labels('Days in training', 'Performance on easy trials (%)')
