...
fig.savefig(os.path.join(
    figpath, "figure4c_psychfuncs_biased_perlab.png"), dpi=600)
plt.close('all')
...
fig.despine(trim=True)
fig.savefig(os.path.join(
    figpath, "figure4c_psychfuncs_biased_perlab_vertical.pdf"))
fig.savefig(os.path.join(
    figpath, "figure4c_psychfuncs_biased_perlab_vertical.png"), dpi=600)
plt.close('all')
...
fig.savefig(os.path.join(
    figpath, "figure4d_chronfuncs_biased_perlab.png"), dpi=600)
plt.close('all')

fig = sns.FacetGrid(behav,
                    col="subject_nickname", col_wrap=8, hue="probabilityLeft",
                    sharex=True, sharey=True, aspect=1)
fig.map(plot_chronometric, "signed_contrast",
        "rt", "subject_nickname").add_legend()
fig.set_axis_labels('Signed contrast (%)', 'Response time (s)')
fig.set_titles("{col_name}")
fig.despine(trim=True)
