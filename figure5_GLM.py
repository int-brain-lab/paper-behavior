...
def run_glm(behav, example, correction=True,  bias=False, cross_validation=True):
    for i, nickname in enumerate(np.unique(behav['subject_nickname'])):
        ...
        if bias is True:
            ...
            behav.loc[behav['subject_nickname'] == nickname, 'bias_n'] = neutral_n.loc[0, 'bias']
            behav.loc[behav['subject_nickname'] == nickname, 'bias_r'] = right_fit.loc[0, 'bias']
            behav.loc[behav['subject_nickname'] == nickname, 'bias_l'] = left_fit.loc[0, 'bias']
        else:
        ...
        data = trials[['index', 'trial_feedback_type', 'signed_contrast',
                       'choice', 'probabilityLeft']].copy()

        # Rewarded choices:
        data.loc[(data['choice'] == 0) & (data['trial_feedback_type'].isnull()), 'rchoice'] = 0
        data.loc[(data['choice'] == -1) & (data['trial_feedback_type'] == -1), 'rchoice'] = 0
        data.loc[(data['choice'] == -1) & (data['trial_feedback_type'] == 1), 'rchoice'] = -1
        data.loc[(data['choice'] == 1) & (data['trial_feedback_type'] == 1), 'rchoice'] = 1
        data.loc[(data['choice'] == 0) & (data['trial_feedback_type'].isnull()), 'rchoice'] = 0
        data.loc[(data['choice'] == 1) & (data['trial_feedback_type'] == -1), 'rchoice'] = 0

        # Unrewarded choices:
        data.loc[(data['choice'] == 0) & (data['trial_feedback_type'].isnull()), 'uchoice'] = 0
        data.loc[(data['choice'] == -1) & (data['trial_feedback_type'] == -1), 'uchoice'] = -1
        data.loc[(data['choice'] == -1) & (data['trial_feedback_type'] == 1), 'uchoice'] = 0
        data.loc[(data['choice'] == 1) & (data['trial_feedback_type'] == 1), 'uchoice'] = 0
        data.loc[(data['choice'] == 0) & (data['trial_feedback_type'].isnull()), 'uchoice'] = 0
        data.loc[(data['choice'] == 1) & (data['trial_feedback_type'] == -1), 'uchoice'] = 1
        ...
        if correction is True:
           data['rchoice+1'] = data['rchoice'].shift(periods=-1).to_numpy()
           data['uchoice+1'] = data['uchoice'].shift(periods=-1).to_numpy()
        ...
        exog = data.copy()
        exog.drop(columns=['trial_feedback_type', 'signed_contrast', 'choice', 'probabilityLeft'],
                  inplace=True)
...
def data_2_X_test (behav, correction=True, bias=True):
        data = behav[['index','trial_feedback_type', 'signed_contrast'
                      'choice', 'probabilityLeft']].copy()
        ...
        # Shift rewarded and unrewarded predictors by one
        data.loc[:, ['rchoice', 'uchoice']] = (data[['rchoice', 'uchoice']]
                                               .shift(periods=1)
                                               .to_numpy())
        ...
        endog = data['choice'].copy()
        exog = data.copy()
        exog.drop(columns=['index', 'trial_feedback_type', 'signed_contrast', 'choice',
...
def plot_psychometric(x, y, col, point=False, line=True, mark='.', al=1, ax=None):
    ...
    df2 = df.groupby(['signed_contrast']).agg({'choice2': 'count', 'choice': 'mean'}).reset_index()
    ...
    if line:
        ...
        sns.lineplot(np.arange(-29, 29),
                     psy.erf_psycho_2gammas(pars, np.arange(-29, 29)), color=col, alpha=al, ax=ax)

        # plot psychfunc: -100, +100
        sns.lineplot(np.arange(-37, -32),
                     psy.erf_psycho_2gammas(pars, np.arange(-103, -98)), color=col, alpha=al, ax=ax)
        sns.lineplot(np.arange(32, 37),
                     psy.erf_psycho_2gammas(pars, np.arange(98, 103)), color=col, alpha=al, ax=ax)
    ...
    ax.set_xticklabels(['-100', ' ', ' ', '0', ' ', ' ', '100'], size='small', rotation=45)
    ax.set_xlim([-40, 40])
    ax.set_ylim([0, 1])
...
plt.close('all')
fig, ax = plt.subplots(1, 2, figsize=(FIGURE_WIDTH / 3.5, FIGURE_HEIGHT * 0.9), sharey='row')

# Plot
sns.lineplot(rsimulation['signed_contrast'], rsimulation['simulation_run'],
             color='k', ci=95, linewidth=0, ax=ax[0])
plot_psychometric(tsimulation['signed_contrast'], tsimulation['choice_right'], 'k',
                  point=True, line=False, mark='o', al=0.5, ax=ax[0])
...
for c, i  in enumerate([20, 50, 80]):
    ...
    sns.lineplot(subset['signed_contrast'], subset['simulation_run'],
                 ci=95, color=cmap[c], linewidth=0, ax=ax[1])
    plot_psychometric(subset1['signed_contrast'], subset1['choice_right'], cmap[c],
                      point=True, line=False, mark='o', al=0.5, ax=ax[1])
...
fig, ax = plt.subplots(1, 3, figsize=(FIGURE_WIDTH / 2, FIGURE_HEIGHT),
                       gridspec_kw={'width_ratios': [3, 2.5, 1.5]})
...
ax[0].plot(bsensory.groupby(['parameter'])['weight'].mean()[['6','12','25','100']],
           olor='black', linewidth=0, marker='_', markersize=8)
...
fig, ax  = plt.subplots(1, 3, figsize=(FIGURE_WIDTH * 0.55, FIGURE_HEIGHT),
                        gridspec_kw={'width_ratios': [3, 2.5, 2.5]})
...
grouped = behav_merge.groupby(
    ['subject_nickname','institution_code','task'])['accu'].mean().reset_index()
...
axbox = sns.boxplot(data=grouped.loc[grouped['task'] == 'traini'], x='institution_code',
                    y='accu', color='white',
                    order=['Lab 1', 'Lab 2', 'Lab 3', 'Lab 4', 'Lab 5', 'Lab 6', 'Lab 7', 'All'])
...
plt.sca(ax[1])
axbox = sns.boxplot(data = grouped.loc[(grouped['task'] == 'biased')],
                    x='institution_code', y='accu', color='white',
                    order=['Lab 1', 'Lab 2', 'Lab 3', 'Lab 4', 'Lab 5', 'Lab 6', 'Lab 7', 'All'])
sns.swarmplot(data=grouped.loc[(grouped['task'] == 'biased') &
                               (grouped['alpha'] == 1)],
              x='institution_code', y=grouped['accu'], color='k',
              order=['Lab 1', 'Lab 2', 'Lab 3', 'Lab 4', 'Lab 5', 'Lab 6', 'Lab 7', 'All'],
              palette=pal)
axbox.artists[-1].set_edgecolor('black')
for j in range(5 * (len(axbox.artists)) + 1, 5 * len(axbox.artists) + 7):
