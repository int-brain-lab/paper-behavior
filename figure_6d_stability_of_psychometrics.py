...
ephys_mice = pd.DataFrame(((subject.Subject()  & 'sex!="U"')  * \
       (behavior_analysis.SessionTrainingStatus() & 'training_status="ready4ephysrig"')).fetch('subject_nickname', as_dict=True))['subject_nickname'].unique()
...
#sns.lineplot(x="level_0", y="bias", data=pars_02_pool,hue="lab_name", ax=ax[0], lw=0.8, \
 #            alpha =0.5, err_kws={'alpha' :0})
sns.lineplot(x="level_0", y="bias", data = pars_02_pool, ax=ax,  color='chocolate')
sns.lineplot(x="level_0", y="bias", data = pars_08_pool, ax=ax,  color='lightseagreen')
#ax[0].legend_.remove()
ax.set_xlim([0, 20])
ax.set_ylim([-30, 30])
ax.set_ylabel('Bias')
green_patch = mpatches.Patch(color='lightseagreen', label='Right Block P(Right Grating) = 0.8')
blue_patch  =  mpatches.Patch(color='chocolate', label='Left Block P(Left Grating) = 0.8')
plt.legend(handles=[green_patch, blue_patch], loc = 'upper left')
ax.set_xlabel('Session Number')
#ax[0].set_ylabel('Bias  20% Right block')
