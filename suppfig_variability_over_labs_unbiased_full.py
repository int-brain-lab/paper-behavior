...
if QUERY is True:
    # query sessions
    from paper_behavior_functions import query_sessions_around_criterion, query_sessions
    from ibl_pipeline import reference, subject, behavior
    use_sessions, _ = query_sessions_around_criterion(criterion='biased',
                                                      days_from_criterion=[0, 15])
    #use_sessions = query_sessions(task='biased')
    use_sessions = use_sessions & 'task_protocol LIKE "%biased%"'  # only get biased sessions   
    b = (use_sessions * subject.Subject * subject.SubjectLab * reference.Lab
         * behavior.TrialSet.Trial & 'trial_stim_prob_left = "0.5"')  # only get 50/50 trials
...
for i, nickname in enumerate(behav['subject_nickname'].unique()):
    ...
    learned.loc[i, 'n_sessions'] = trials['session_uuid'].unique().size
...
plt.savefig(join(figpath, 'suppfig_all_metrics_per_lab_unbiased_full.pdf'))
plt.savefig(join(figpath, 'suppfig_all_metrics_per_lab_unbiased_full.png'), dpi=300)
...
plt.tight_layout()
sns.despine(trim=True)
plt.savefig(join(figpath, 'suppfig_metrics_per_lab_unbiased_full.pdf'))
plt.savefig(join(figpath, 'suppfig_metrics_per_lab_unbiased_full.png'), dpi=300)
